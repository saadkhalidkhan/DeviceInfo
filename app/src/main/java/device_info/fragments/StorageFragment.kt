...
//        handler.postDelayed(runnable, 0)
    ...
    private fun setUpStorageDetails() {
        ...
        tvUsedIntMemory?.text =
            getString(R.string.used_memory) + "\t" + formatSize(usedInternalValue)
        ...
        donutInternalStorage?.progress =
            Methods.calculatePercentage(usedInternalValue.toDouble(), totalInternalValue.toDouble())
                .toFloat()
        ...
        if (Methods.getExternalMounts().size > 0) {
            ...
            tvUsedExtMemory?.text =
                getString(R.string.used_memory) + "\t" + formatSize(usedExternalValue)
            tvFreeExtMemory?.text = getString(R.string.free) + "\t" + formatSize(freeExternalValue)
            tvTotalExtMemory?.text =
                getString(R.string.used) + "\t" + formatSize(totalExternalValue)

            donutExternalStorage?.progress = df.format(
    ...
    private fun showRAMUsage() {
        ...
        donutRAMUsage?.progress =
            Methods.calculatePercentage(usedRamValue.toDouble(), totalRamValue.toDouble()).toFloat()
    }
