...
import device_info.models.SimInfoModel
import device_info.activity.HomeActivity
import device_info.adapters.SimListAdapter
...
class SimFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        rvSimData?.layoutManager = androidx.recyclerview.widget.LinearLayoutManager(mActivity)
    ...
    private fun checkCameraPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            val hasWriteCameraPermission =
                mActivity.checkSelfPermission(Manifest.permission.READ_PHONE_STATE)
            if (hasWriteCameraPermission != PackageManager.PERMISSION_GRANTED) {
                requestPermissions(
                    arrayOf(Manifest.permission.READ_PHONE_STATE),
                    KeyUtil.KEY_READ_PHONE_STATE
                )
            } else {
            ...
            KeyUtil.KEY_READ_PHONE_STATE -> if (permissions.isNotEmpty()) {
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (ActivityCompat.shouldShowRequestPermissionRationale(
                        ...
                        Toast.makeText(
                            mActivity,
                            "Need to grant account Permission",
                            Toast.LENGTH_LONG
                        ).show()
                    }
                }
    ...
    private fun retrieveSimInformation(telephonyManager: TelephonyManager) {
        if (telephonyManager != null && isSimAvailable(
                mActivity,
                0
