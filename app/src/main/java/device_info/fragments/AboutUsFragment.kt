...
class AboutUsFragment : BaseFragment(), View.OnClickListener {
    ...
    private fun initOnclickListner() {
        ...
        rbRatingBar?.onRatingBarChangeListener =
    ...
    private fun callIntent() {
        val intent =
            Intent(Intent.ACTION_CALL, Uri.parse("tel:" + getString(R.string.company_phone)))
    ...
    private fun checkCameraPermission() {
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            val hasWriteCameraPermission =
                mActivity?.checkSelfPermission(Manifest.permission.CAMERA)
            if (hasWriteCameraPermission != PackageManager.PERMISSION_GRANTED) {
                requestPermissions(
                    arrayOf(Manifest.permission.CALL_PHONE),
                    KeyUtil.KEY_CALL_PERMISSION
                )
            } else {
                callIntent()
            }
