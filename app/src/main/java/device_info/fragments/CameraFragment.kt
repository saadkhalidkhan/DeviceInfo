...
import device_info.adapters.CamaraAdapter
...
class CameraFragment : BaseFragment(), View.OnClickListener {
    ...
    private var rvCameraFeatures: androidx.recyclerview.widget.RecyclerView? = null
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        rvCameraFeatures?.layoutManager =
            ...
            if (getCharacteristicsValue(key, characteristics) != "") {
                if (key.name.split(".").size == 4) {
                    lists.add(
                        FeaturesHWModel(
                            keyNm[3],
                            getCharacteristicsValue(key, characteristics)
                        )
                    )
                } else {
                    lists.add(
