...
//        val view = inflater.inflate(R.layout.fragment_sensors_detail, container, false)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        sensorManager?.registerListener(
            this,
            sensorType?.let { sensorManager?.getDefaultSensor(it) },
    ...
    private fun displayAccelerometer(event: SensorEvent?) {
        ...
        if (sensorManager?.getDefaultSensor(Sensor.TYPE_ACCELEROMETER)?.type === sensorType
            ...
            tvXAxis?.text =
                (Html.fromHtml("X: " + formatter.format(x) + getString(R.string.ms) + "<small><sup>2</sup></small>"))
            tvYAxis?.text =
                (Html.fromHtml("Y: " + formatter.format(y) + getString(R.string.ms) + "<small><sup>2</sup></small>"))
            tvZAxis?.text =
                (Html.fromHtml("Z: " + formatter.format(z) + getString(R.string.ms) + "<small><sup>2</sup></small>"))
        }
        /*** Magnetic sensors */
            ...
            if (sensorManager?.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD)?.type === (sensorType)) {
                ...
                tvXAxis?.text =
                    (getString(R.string.geomagnetic_field) + "X: " + formatter.format(x) + getString(
                        R.string.mu_tesla
                    ))
                tvYAxis?.text = ("Y: " + formatter.format(y) + getString(R.string.mu_tesla))
                tvZAxis?.text = ("Z: " + formatter.format(z) + getString(R.string.mu_tesla))
        ...
        /*** Gyroscope sensors */
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                tvXAxis?.text = Methods.getSpannableSensorText(
                    mActivity,
                    getString(R.string.angular_speed) + "X: " + formatter.format(x) + getString(R.string.rad)
                )
            }
        ...
        /*** Pressure sensor (Barometer) */
            ...
            if (event != null) {
                tvXAxis?.text =
                    (getString(R.string.pressure) + event.values[0] + getString(R.string.hpa))
            }
        }
        /*** Step counter sensor
        Proximity sensor
        Light sensor */
            ...
            if (event != null) {
                if (event.sensor.type == Sensor.TYPE_RELATIVE_HUMIDITY && event.values[0] < sensorManager?.getDefaultSensor(
                    ...
                    tvXAxis?.text =
                        (getString(R.string.proximity_sensor) + event.values[0].toString() + getString(
                            R.string.cm
                        ))
                } else {
                    tvXAxis?.text =
                ...
                if (event.sensor.type == Sensor.TYPE_RELATIVE_HUMIDITY) {
                    tvXAxis?.text =
                        (getString(R.string.humidity_sensor) + event.values[0].toString() + getString(
                            R.string.percentage
                        ))
                ...
                if (event.sensor.type == Sensor.TYPE_LIGHT) {
                    tvXAxis?.text =
                        (getString(R.string.illuminance) + event.values[0].toString() + getString(R.string.lx))
                }


                if (event.sensor.type == Sensor.TYPE_AMBIENT_TEMPERATURE && KeyUtil.KEY_LAST_KNOWN_HUMIDITY != 0f) {
                    val temperature = event.values[0]
                    val absoluteHumidity = Methods.calculateAbsoluteHumidity(
                        temperature,
                        KeyUtil.KEY_LAST_KNOWN_HUMIDITY
                    )
                    tvXAxis?.text =
                        (getString(R.string.absolute_humidity_temperature_sensor) + formatter.format(
                            absoluteHumidity
                        ) + getString(R.string.percentage))
                    val dewPoint =
                        Methods.calculateDewPoint(temperature, KeyUtil.KEY_LAST_KNOWN_HUMIDITY)
                    tvYAxis?.text =
                        (getString(R.string.due_point_temperature) + formatter.format(dewPoint) + getString(
                            R.string.percentage
                        ))
                }
    ...
    private fun displaySensorsDetails(sensorManager: SensorManager) {
        ...
        tvResolution?.text =
            sensorType?.let { Html.fromHtml(sensorManager.getDefaultSensor(it)?.resolution.toString() + " m/s" + "<small><sup>2</sup></small>") }
        tvPower?.text =
            sensorType?.let { sensorManager.getDefaultSensor(it)?.power.toString() + getString(R.string.ma) }
        tvMaximumRange?.text =
            sensorType?.let { Html.fromHtml(sensorManager.getDefaultSensor(it)?.maximumRange.toString() + " m/s" + "<small><sup>2</sup></small>") }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            tvSensorId?.text = sensorType?.let { sensorManager.getDefaultSensor(it)?.id.toString() }
            tvIsDynamic?.text =
                sensorType?.let { sensorManager.getDefaultSensor(it)?.isDynamicSensor.toString() }
            tvIsWakeUpSensor?.text =
                sensorType?.let { sensorManager.getDefaultSensor(it)?.isWakeUpSensor.toString() }
            tvReportingMode?.text =
                sensorType?.let { sensorManager.getDefaultSensor(it)?.reportingMode.toString() }
        } else {
            tvSensorId?.text = "-"
            tvIsDynamic?.text = "-"
            tvIsWakeUpSensor?.text = "-"
